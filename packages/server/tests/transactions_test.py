...
class TestTransactionsMethods(unittest.TestCase):
    ...
    def test_buy_transaction_execution(self):
        ...
        self.assertTrue(
            "Tech Co" in self.portfolio.portfolios[self.portfolio_name]
        )  # Adjust based on actual implementation
    ...
    def test_buy_transaction_insufficient_funds(self):
        ...
        self.portfolio.initialize_balance(
            self.portfolio_name
        )  # Reset or initialize the balance if  Portfolio supports it
        self.portfolio.change_balance(
            self.portfolio_name, low_initial_balance, 0
        )  # Set the low balance
