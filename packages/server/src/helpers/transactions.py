...
*Row Attributes: (Company Name, QTY / Quantity, Unit Price, Daily P/L, Total P/L) #replace with API
   ...
   """
    ...
    def validate_portfolioname(self, portfolio_name: str):
        """
        ...
        if len(portfolio_name) < Portfolio.PORTFOLIO_LENGTH_MIN:
            raise ValueError(Portfolio.ERROR_MESSAGES[Portfolio.ERROR_TYPES.PORTFOLIO_SHORT])

        if len(portfolio_name) > Portfolio.PORTFOLIO_LENGTH_MAX:
            raise ValueError(Portfolio.ERROR_MESSAGES[Portfolio.ERROR_TYPES.PORTFOLIO_LONG])

        for char in portfolio_name:
            if char not in Portfolio.PORTFOLIO_VALID_CHARACTERS:
                raise ValueError(
                    Portfolio.ERROR_MESSAGES[Portfolio.ERROR_TYPES.PORTFOLIO_INVALID_CHARACTERS]
                )
    ...
    def remove_portfolio(self, portfolio_name: str):
        ...
        if portfolio_name not in self.portfolios:
            raise KeyError("Portfolio does not exist.\nPlease try again and input a portfolio name that exists inside of the Fintasy Databases.")
        del self.portfolios[portfolio_name]
        return True


    def add_data(self, portfolio_name: str, company_name: str, qty: int, unit_price: float, daily_PL: float, total_PL: float): #unit_price, dailyPL, and totalPL will be received through API later
        """
        Args:
            portfolio_name (str): The portfolio name must exist inside of the dictionary.
        ...
        if len(company_name) < Portfolio.PORTFOLIO_LENGTH_MIN:
            raise ValueError("Company name must contain one or more characters inside of its name.")
        ...
        if qty < 0:
            raise ValueError(f"The quantity of {company_name}'s stock(s) must be greater than zero inside of this portfolio.")

        if unit_price < 0:
            raise ValueError(f"The unit price of {company_name}'s stock(s) must be greater than zero inside of this portfolio.")

        if daily_PL < 0:
            raise ValueError(f"The daily PL of {company_name}'s stock(s) must be greater than zero inside of this portfolio.")
